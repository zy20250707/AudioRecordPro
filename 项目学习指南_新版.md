# AudioRecordMac 项目学习指南 (新版 - 2025年更新)

## 项目概述

AudioRecordMac 是一个基于 Swift 开发的**专业级** macOS 音频录制应用程序，采用现代化的多轨道录制架构，支持：

### 核心能力
- 🎙️ **多音源同时录制** - 可同时录制麦克风、系统音频和多个应用程序
- 🎤 **麦克风录制** (AVAudioEngine) - 高质量音频输入
- 🔊 **系统音频录制** (CoreAudio Process Tap) - macOS 14.4+ 新技术
- 📱 **特定应用程序录制** - 精确捕获指定应用的音频输出
- 🔀 **多进程混音录制** - 同时录制最多3个应用程序并混音
- ✨ **融合录音** - 系统音频 + 麦克风实时混音到单文件
- ☑️ **勾选式进程选择** - 直观的勾选框界面，支持多选
- 📊 **专业电平表** - 实时音频监控和可视化
- 🎵 **音频格式转换** - 内置 WAV 到 MP3 导出功能
- 📁 **文件管理系统** - 完整的录音文件管理界面
...
└───┬─────────────────┬───────────────────┬───────────────────┘
    ...
    ▼                 ▼                   ▼
┌─────────┐   ┌─────────────┐   ┌─────────────────┐
│Sidebar  │   │ TracksView  │   │ ControlPanelView│
│View     │   │ (轨道视图)  │   │   (控制面板)     │
│         │   │             │   │                 │
│ ┌─Tab─┐ │   │ ┌─Track 1─┐ │   │ ┌─录制控制─┐   │
│ │音频 │ │   │ │🎤麦克风 │ │   │ │ 开始/停止 │   │
...
**视图层级特点：**
- **Tab 系统**: 侧边栏支持多标签切换（音频录制/文件列表）
- **多轨道显示**: 实时显示所有录制源的电平表
- **动态更新**: 根据用户选择动态添加/移除轨道
- **图标支持**: 自动加载应用程序图标
...
**项目最重要的创新功能**，支持同时录制多个音源并生成独立的音频文件：

```swift
// 多音源录制架构
audioRecorderController.startMultiSourceRecording(
    wantMic: true,          // 录制麦克风
    wantSystem: true,       // 录制系统音频
    wantProcess: true,      // 录制特定进程
    targetPID: qqmusicPID   // 目标进程PID
)
```
...
### 2. CoreAudio Process Tap 技术 (核心方案)
...
- ✅ 支持多进程混音录制
- ✅ 高质量音频捕获（无损）
- ✅ 动态采样率适配（44.1kHz/48kHz）
- ✅ 双 API 支持（C API + Swift API）
...
**一键混音录制**，将系统音频和麦克风实时混合到一个文件：

```swift
// 融合录音架构
MixedAudioRecorder {
    ProcessTapRecorder (系统音频) 
    + 
    AVAudioEngine (麦克风)
    ↓
    实时混音算法 (0.6 * 系统 + 0.4 * 麦克风)
    ↓
    mixed_appname+mic_timestamp.wav
}
```
...
- ✅ 单文件输出，无需后期合成
- ✅ 实时混音，零延迟
- ✅ 智能命名 (`mixed_qqmusic+mic_2025-01-15.wav`)
- ✅ 防削波处理，保证音质
- ✅ UI 一键开关，简单易用
...
```
src/Controllers/
├── MainViewController.swift          # 主控制器，协调各个组件，处理融合录音
├── AudioRecorderController.swift     # 录音控制器工厂类，支持混音模式
...
├── MixedAudioRecorder.swift          # 融合录音器 (Process Tap + 麦克风) ⭐️ 新增
...
| 特性 | 麦克风录制 | 系统音频录制 | Process Tap录制 | **融合录音** ⭐️ |
|------|------------|--------------|-----------------|-----------------|
| **数据源** | 麦克风硬件输入 | 系统音频混音 | 特定进程音频 | **系统音频+麦克风** |
| **技术栈** | AVAudioEngine | ScreenCaptureKit | CoreAudio Process Tap | **Process Tap + AVAudioEngine** |
| **权限要求** | 麦克风权限 | 屏幕录制权限 | 屏幕录制权限 | **麦克风+屏幕录制权限** |
| **系统版本** | macOS 10.0+ | macOS 12.3+ | macOS 14.4+ | **macOS 14.4+** |
| **音频质量** | 原始输入质量 | 系统混音质量 | 原始进程质量 | **高质量混音** |
| **延迟** | 低延迟 | 中等延迟 | 低延迟 | **低延迟** |
| **精确度** | 高 | 低 | 高 | **高** |
| **复杂度** | 简单 | 中等 | 复杂 | **高级** |
| **文件格式** | WAV/M4A | WAV/M4A | WAV (32-bit Float) | **WAV (32-bit Float)** |
| **采样率** | 固定 48kHz | 固定 48kHz | 动态适配 | **统一 48kHz** |
| **文件数量** | 1 | 1 | 1 | **1 (混音)** |
| **适用场景** | 语音录制、采访 | 系统音频录制 | 特定应用音频录制 | **会议录制、直播、教学** |
...
3. **点击录制按钮**
4. **完成后停止录制**
5. **查看生成的混音文件**: `mixed_appname+mic_timestamp.wav`
...
```
勾选: 系统音频 + 麦克风 + 实时混音
结果: 会议中其他人的声音 + 你的声音，混合在一个文件中
文件: mixed_system+mic_2025-01-15_14-30.wav
```

#### 场景2: 游戏直播录制
```
勾选: 选择游戏进程 + 麦克风 + 实时混音
结果: 游戏音效 + 你的解说，混合在一个文件中
文件: mixed_game+mic_2025-01-15_14-30.wav
```

#### 场景3: 音乐教学
```
勾选: QQ音乐进程 + 麦克风 + 实时混音
结果: 播放的音乐 + 你的讲解，混合在一个文件中
文件: mixed_qqmusic+mic_2025-01-15_14-30.wav
```
...
- ✅ 提供了高性能的实时音频数据处理方案
- ✅ 支持多进程混音录制
- ✅ **创新的融合录音功能** (系统音频 + 麦克风实时混音) ⭐️

CoreAudio Process Tap 技术为 macOS 音频录制开辟了新的可能性，支持高质量的应用程序音频录制，是未来 macOS 音频应用开发的重要方向。本项目为这一技术的实际应用提供了完整的参考实现。

**最新更新 (2025年):**
- ✨ **新增融合录音功能** - 系统音频与麦克风实时混音
- 🎵 智能文件命名 - `mixed_appname+mic_timestamp.wav`
- 🔧 优化混音算法 - 防削波处理，保证音质
- 📊 扩展 UI 功能 - 混音开关一键切换
- 🎯 完善技术文档 - 《融合录音技术方案.md》

**技术栈亮点：**
- Swift + CoreAudio + AVFoundation
- Process Tap + AVAudioEngine 双引擎
- 实时混音 + 32-bit Float 高精度
- 模块化架构 + 工厂模式
- 完整的权限管理 + 错误处理
